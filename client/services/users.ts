...
export async function createUser(input: UserCreateRequest): Promise<User> {
  const headers = { "Content-Type": "application/json", ...authHeaders() } as Record<string, string>;
  const key = cacheKeyFor(LIST_URL);
  ...
  // Update cache
  const current = (await getCached<User[]>(key)) ?? [];
  await setCached(key, [created, ...current.filter((u) => !String(u.id).startsWith('local-'))]);
  return created;
}
...
export async function updateUser(id: string, patch: UserUpdateRequest): Promise<User> {
  ...
  if (!isOnline()) {
    ...
    const current = (await getCached<User[]>(key)) ?? [];
    const updated = current.map((u) => (u.id === id ? { ...u, ...patch } as User : u));
    await setCached<User[]>(key, updated);
    await enqueue({ url: `${LIST_URL}/${id}`, method: "PUT", headers, body: patch });
  ...
  const current = (await getCached<User[]>(key)) ?? [];
  await setCached<User[]>(key, current.map((u) => (u.id === id ? updated : u)));
  return updated;
}
...
export async function deleteUserApi(id: string): Promise<void> {
  ...
  if (!isOnline()) {
    ...
    await enqueue({ url: `${LIST_URL}/${id}`, method: "DELETE", headers, body: null });
    return;
  }
